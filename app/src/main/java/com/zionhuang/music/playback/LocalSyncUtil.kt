...
class LocalSyncUtil @Inject constructor(
    ...
    suspend fun exportDownloadToMediaStore(songId: String) = withContext(Dispatchers.IO) {
        ...
        // IMPORTANT: We include the YouTube ID to reconstruct the Local library perfectly later
        val fileNameBase = "[$songId] $artistName - $title".replace(Regex("[\\\\/:*?\"<>|]"), "_")
        ...
        if (audioUri != null) {
            try {
                resolver.openOutputStream(audioUri)?.use { output ->
                    ...
                    val dataSpec = DataSpec.Builder().setKey(songId).setUri(songId.toUri()).build()
                ...
                e.printStackTrace()
                // Do not throw so we can at least try lyrics
            }
        }

        // 2. Export Lyrics (.lrc file)
        ...
        if (lyricsEntity != null && lyricsEntity.lyrics != LyricsEntity.LYRICS_NOT_FOUND) {
            val lyricsValues = ContentValues().apply {
                ...
                put(MediaStore.MediaColumns.MIME_TYPE, "text/plain") // .lrc mimetype
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    put(MediaStore.MediaColumns.RELATIVE_PATH, relativePath)
                }
            }
            ...
            try {
                val lyricsUri = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ...
                    val dir = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "InnerTune")
                    val file = File(dir, lyricsFileName)
                    if (!file.exists()) file.createNewFile()
                    file.toUri()
                }

                if (lyricsUri != null) {
                    resolver.openOutputStream(lyricsUri)?.use { output ->
